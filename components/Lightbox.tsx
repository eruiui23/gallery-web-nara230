...
export default function Lightbox({ photos, currentIndex, onClose, onNavigate }: Props) {
    ...
    // ONLY return null if the window hasn't mounted yet to avoid Next.js hydration errors
    if (!mounted) return null;

    return createPortal(
        <AnimatePresence>
            {/* The condition moves INSIDE AnimatePresence so it can animate the exit! */}
            {currentIndex !== null && (
                <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    ...
                    // Cranked the z-index to 99999 to permanently bury the navbar
